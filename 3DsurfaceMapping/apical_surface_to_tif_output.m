...
lastThreeChars = extractAfter(fullpath, strlength(fullpath) - 3);
...
if lastThreeChars == "czi" || lastThreeChars =="oir" || lastThreeChars == "nd2"
    ...
    elseif lastThreeChars =="oir"
        ...
        highest_intensity=max(max(max(separateChannels(:,:,:,2))));      
        for i  = 1: size(separateChannels(:,:,:,2),3)
            plane=separateChannels(:,:,i,2);

            %Trying to scale the .oir file intensities similar to .czi
            separateChannels(:,:,i,2) = imadjust(plane)*255;  
        end
...
%data again.
[~,verts] = extractIsosurface(separateChannels(:,:,:,2), isovalue);
